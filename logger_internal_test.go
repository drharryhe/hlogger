...
func TestPrintMessage_NoColor(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			internalResetLogConfig()
			stdout, stderr := internalRedirectOutput()
			defer internalResetLogOutput()
...
func internalRedirectOutput() (*bytes.Buffer, *bytes.Buffer) {
